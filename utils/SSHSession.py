...
class SSHSession(object):
    ...
    def put_all(self,localpath,remotepath):
        ...
        print(os.path.split(localpath)[0])
    ...
    def sftp_fuzzywalk(self, remotepath, fuzzy):
        path=remotepath
        files=[]
        folders=[]
        for f in self.sftp.listdir_attr(remotepath):
            if fuzzy in f.filename:
                if S_ISDIR(f.st_mode):
                    folders.append(f.filename)
                else:
                    files.append(f.filename)
        ...
        for folder in folders:
            new_path=os.path.join(remotepath,folder)
            for x in self.sftp_fuzzywalk(new_path,fuzzy):
                yield x
    
    def local_fuzzywalk(self, localpath, fuzzy):
        path=localpath
        files=[]
        folders=[]
        ...
        yield path,folders,files
        for folder in folders:
            new_path=os.path.join(local,folder)
            for x in self.local_fuzzywalk(new_path,fuzzy):
                yield x

    def get_fuzzy(self, remotepath, localpath, fuzzy):
        self.sftp.chdir(remotepath)
        parent=os.path.split(remotepath)[1]
        for walker in self.sftp_fuzzywalk(parent, fuzzy):
            for file in walker[2]:
                self.get(os.path.join(walker[0],file),os.path.join(localpath,file))
    
    def put_fuzzy(self, localpath, remotepath, fuzzy):
        for walker in os.walk(localpath):
            for folder in walker:
                for file in folder:
                    if fuzzy in file:
                        print(file)
                        self.put(file,os.path.join(remotepath,file))
